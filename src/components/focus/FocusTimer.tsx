...
import {
  ...
  Loader2, Check, GripVertical, RotateCcw, Bell,
} from 'lucide-react'
...
// Three-tone ascending chime via Web Audio API — works on Mac and mobile browser
function playAlarm() {
  try {
  ...
  } catch { /* blocked by browser autoplay policy — no-op */ }
}
    ...
    <div className="flex flex-col items-center">
      <div className="relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-1">
          ...
          <span className="text-xs text-slate-400 max-w-[130px] text-center truncate mt-1">
            {label}
          </span>
        </div>
      </div>
    ...
    <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2">
      ...
      <div className="flex items-center gap-1 shrink-0">
        <input
          ...
          value={mins}
          onChange={e => onChange('duration', Number(e.target.value) * 60 + secs)}
          className="w-10 rounded border border-white/10 bg-white/5 px-1.5 py-0.5 text-center text-sm text-white focus:outline-none focus:border-violet-500"
        />
        <span className="text-xs text-slate-600">m</span>
        <input
          type="number" min={0} max={59}
          value={secs}
          onChange={e => onChange('duration', mins * 60 + Number(e.target.value))}
          className="w-10 rounded border border-white/10 bg-white/5 px-1.5 py-0.5 text-center text-sm text-white focus:outline-none focus:border-violet-500"
        />
...
export default function FocusTimer() {
  ...
  // Run display state (for rendering)
  ...
  function togglePause() {
    if (pausedRef.current) {
      // Resume: shift endTime forward by how long we were paused
  ...
  if (mode === 'run' && runSeq) {
    ...
    const ringColor = done    ? 'text-emerald-400'
      ...
      <div className="flex flex-col items-center gap-5">
        ...
        {done ? (
          ...
          <div className="flex flex-col items-center gap-4 py-8">
            ...
            <Button onClick={stopTimer} className="mt-2 bg-violet-600 hover:bg-violet-700 text-white px-8">
              Done
            </Button>
          </div>
        ) : (
          <>
            ...
            <p className="text-xs text-slate-500">
              Step {stepIdx + 1} of {runSeq.steps.length}
            </p>
            ...
            {'Notification' in window && Notification.permission === 'denied' && (
              <div className="flex items-center gap-2 rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-400">
                ...
                Enable browser notifications to receive alerts when steps finish.
              </div>
            )}
            ...
            <div className="w-full space-y-1.5 pt-2">
              {runSeq.steps.map((s, i) => (
                <div key={i} className={cn(
                  ...
                  i === stepIdx && 'border border-violet-500/20 bg-violet-500/8 text-white font-medium',
                  ...
                  {i < stepIdx ? (
                    ...
                    <div className={cn(
                      ...
                      paused ? 'bg-slate-500' : 'bg-violet-400 animate-pulse'
                    )} />
                  ) : (
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-semibold text-white">New Sequence</h2>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            onKeyDown={e => { if (e.key === 'Enter') saveSequence() }}
        ...
        {steps.length > 0 && (
          <p className="text-xs text-slate-500 text-right">
            Total time: <span className="text-slate-400 font-medium">{fmtDuration(totalSecs(steps))}</span>
          </p>
        )}
    ...
    <div className="space-y-4">
      ...
      {'Notification' in window && Notification.permission === 'default' && (
        <button
          ...
          className="flex w-full items-center gap-2 rounded-lg border border-violet-500/20 bg-violet-500/8 px-3 py-2.5 text-xs text-violet-300 hover:bg-violet-500/15 transition-all text-left"
        >
          <Bell className="h-3.5 w-3.5 shrink-0" />
          Allow notifications to get alerts when each timer step ends
        </button>
      )}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {seq.steps.map((s, i) => (
                        <span key={i}
                          className="text-[10px] rounded-full bg-white/5 border border-white/8 px-2 py-0.5 text-slate-400">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="text-slate-700 opacity-0 group-hover:opacity-100 hover:text-red-400 transition-all"
                    >
