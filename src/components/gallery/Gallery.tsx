...
export default function Gallery({ initialItems, userId }: { initialItems: GalleryItem[]; userId: string }) {
  ...
  useEffect(() => {
    async function onPaste(e: ClipboardEvent) {
      const imageItems = Array.from(e.clipboardData?.items ?? []).filter(it => ACCEPTED_TYPES.test(it.type))
  ...
  const uploadFiles = useCallback(async (files: File[]) => {
    ...
    setUploading(true)
    setError(null)
    setUploadNames(valid.map(f => f.name || 'screenshot'))
    ...
    for (const file of valid) {
      ...
      const { data: row, error: dbErr } = await supabase
        .from('user_gallery')
        .insert({ user_id: userId, storage_path: path, url: publicUrl, caption: null, tags: [] })
        .select()
        .single()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {items.length} image{items.length !== 1 ? 's' : ''} · drag-drop, click, or ⌘V to paste
          </p>
        </div>
        <button
      ...
      >
        <ImageIcon className={cn('h-7 w-7', dragging ? 'text-violet-400' : 'text-slate-600')} />
        <p className="text-sm text-slate-500 text-center px-4">
          {dragging
            ? 'Drop images here'
            : 'Drag & drop images · click to browse · paste a screenshot with ⌘V / Ctrl+V'}
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="py-20 text-center">
          ...
          <p className="text-slate-400 text-sm">
            {filterTag ? `No images tagged #${filterTag}` : 'No images yet — upload or paste a screenshot to get started'}
          </p>
        </div>
      ) : (
