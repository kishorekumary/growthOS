...
const STARTERS = [
  'What book would help me build better habits?',
  'Summarise the key ideas from "Thinking, Fast and Slow"',
  'What should I read after finishing Atomic Habits?',
]

export default function BooksCoach() {
  ...
  async function send(text?: string) {
    ...
    try {
      const res = await fetch('/api/ai/books-coach', {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col h-[520px]">
      <h2 className="font-semibold text-white mb-4 shrink-0">Book Coach</h2>
        ...
        {messages.length === 0 && (
          <div className="space-y-4">
            <div className="flex flex-col items-center py-8 text-center">
              ...
              <p className="text-sm font-medium text-white">Your Book Coach</p>
              <p className="text-xs text-slate-500 mt-1">
                Ask for recommendations, summaries, or key insights.
              </p>
            </div>
      ...
      <div className="flex gap-2 pt-3 shrink-0 border-t border-white/10 mt-3">
        <input
          ...
          placeholder="Ask your book coach…"
          disabled={loading}
